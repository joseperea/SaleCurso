...
namespace Sale.API.Data
{
    public class SeedDb
    {
        ...
        public async Task SeedAsync()
        {
            ...
            await CheckUserAsync("1010", "Jose", "Valencia", "jose.valencia@yopmail.com", "322 311 4620", "Calle Luna Calle Sol", "Jose.jpg", UserType.Admin);
            await CheckUserAsync("2020", "lorena", "Valencia", "lorena@yopmail.com", "322 311 4620", "Calle Luna Calle Sol", "Lorena.jpg", UserType.User);
            await CheckUserAsync("3030", "titina", "Valencia", "titina@yopmail.com", "322 311 4620", "Calle Luna Calle Sol", "titna.jpg", UserType.User);
            await CheckUserAsync("4040", "negro", "Valencia", "negro@yopmail.com", "322 311 4620", "Calle Luna Calle Sol", "negro.jpg", UserType.User);
            await CheckUserAsync("5050", "manchas", "Valencia", "manchas@yopmail.com", "322 311 4620", "Calle Luna Calle Sol", "Manchas.jpg", UserType.User);
        ...
        private async Task AddProductAsync(string name, decimal price, float stock, List<string> categories, List<string> images)
        {
            ...
            foreach (string? image in images)
            {
                ...
                {
                    filePath = $"{Environment.CurrentDirectory}\\SaleImagenes\\products\\{image}";
                }
                else
                {
                    filePath = $"{Environment.CurrentDirectory}/SaleImagenes/products/{image}";
        ...
        private async Task<User> CheckUserAsync(string document, string firstName, string lastName, string email, string phone, string address, string image, UserType userType)
        {
            ...
            if (user == null)
            {
                ...
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                {
                    filePath = $"{Environment.CurrentDirectory}\\SaleImagenes\\users\\{image}";
                }
                else
                {
                    filePath = $"{Environment.CurrentDirectory}/SaleImagenes/users/{image}";
